<?php
	...
	if (isset($_POST["login_submit"]) && !is_null($db)) {
		...
		$statement = $db->prepare("INSERT INTO messages(id, username, ip_addr, message, timestmp) VALUES ((SELECT MAX(id)+1 FROM (SELECT * FROM messages) AS m), ?, ?, ?, CURRENT_TIMESTAMP)");
		$statement->execute(array($_SESSION["username"], $_SERVER['REMOTE_ADDR'], substr(preg_replace("/[^a-zA-Z _\-.,;]/i", "", $_POST["message"]), 0, 64)));
	}
?>
<!DOCTYPE html>
<html>
				...
				<div style="border-style: solid; padding: 15px 15px;">
					<?php
						if (isset($_SESSION["login"]) && $_SESSION["login"] === "true") {
							echo "<p>You are logged in as: " . htmlentities($_SESSION["username"]) . "</p>";
							echo "<form action=\"index.php\" method=\"POST\"><input type=\"submit\" name=\"logout_submit\" value=\"Logout\" /></form>";
						} else {
